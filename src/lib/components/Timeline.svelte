...
<script lang="ts">
  ...
  // 获取连接线颜色
  function getLineColor(prevStep: WorkflowStep | null, nextStep: WorkflowStep | null): string {
          ...
          <div class="relative flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0 pb-2">
              <div class="flex items-center justify-between mb-1">
                ...
                <span class="text-sm text-gray-500">{formatTime(step.operateTime)}</span>
              </div>
              ...
              {#if step.comment}
                <div class="mt-2 p-2 bg-gray-50 rounded-lg text-sm text-gray-600">
                  <span class="font-medium text-gray-500">备注：</span>{step.comment}
                </div>
              {/if}
            </div>
          </div>
